...
public class BrowserPoolConfig {
  ...
  @Bean
  public BrowserPool browserPool(BrowserPoolProperties properties) throws Exception {
    ...
    config.setMaxTotal(properties.maxTotal());
    config.setTestOnCreate(true);
    config.setTestOnReturn(true);

    @SuppressWarnings("java:S2095")
    final var pool = new BrowserPool(new BrowserFactory(), config);
    pool.addObjects(properties.minIdle());
    browserPool = pool;
    return browserPool;
  }
